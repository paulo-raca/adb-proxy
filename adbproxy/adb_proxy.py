...
from .endpoint import Endpoint, StreamReader, StreamWriter, spawn
...
class AdbProxy:
    ...
    async def open_stream(self, *name: str) -> tuple[StreamReader, StreamWriter]:
        ...
        async def on_connected(r: StreamReader, w: StreamWriter) -> None:
            ...
            proxy_task[0] = asyncio.create_task(
                AdbProxy(
                    ...
                    device_endpoint=remote_endpoint,
                    device_id=device_id,
...
async def connect(
    *,
    ...
    host_adb_addr: tuple[str, int] = ("localhost", 5037),
    device_adb_addr: tuple[str, int] = ("localhost", 5037),
    ssh_client: SSHClientConnection | None = None,
) -> None:
    ...
    wait_for: Callable[[str], Awaitable[None]] | None = None
    if connect_cmd:

        async def wait_for(serial: str) -> None:
            env = dict(os.environ)
    ...
    return await AdbProxy.attach_raw(
        local_endpoint=local_endpoint,
        remote_endpoint=remote_endpoint,
        device_id=device_id,
        reverse_connection_supported=adb_reverse_supported,
        wait_for=wait_for,
    )
...
async def connect_reverse(*, server_address: dict[str, Any], ssh_client: SSHClientConnection | None = None, **kwargs: Any) -> None:
