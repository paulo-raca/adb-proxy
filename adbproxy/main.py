...
async def main():
    ...
    listen_reverse_base_parser.add_argument("listen_address", type=ssh_config, nargs='?', default={"host": "0.0.0.0"}, help="Server address where the reverse SSH server will be bound")
    listen_reverse_base_parser.add_argument("--upnp", action='store_true', help="Uses UPNP to setup the Internet Gateway to receive incoming connections")
    listen_reverse_base_parser.add_argument("--ngrok", dest="ssh_tunnels", action="append_const", const={"host": "tunnel.us.ngrok.com", "port": 22, "known_hosts": None, "password": ''}, help="Uses Ngrok as a Gateway")
    ...
    parser_listen_reverse = subparsers.add_parser('listen-reverse', parents=[listen_reverse_base_parser], help='Awaits reverse connections from devices')
